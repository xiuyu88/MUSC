...
# 导入数据
import datasets.old_mvtec as old_mvtec
from datasets.old_mvtec import _CLASSNAMES as _CLASSNAMES_mvtec_ad
...
class MuSc():
    ...
    def load_datasets(self, category, divide_num=1, divide_iter=0):
        ...
        if self.dataset == 'visa':
            ...
            test_dataset = old_mvtec.MVTecDataset(source=self.path, split=old_mvtec.DatasetSplit.TEST,
                                            classname=category, resize=self.image_size, imagesize=self.image_size, clip_transformer=self.preprocess,
                                                divide_num=divide_num, divide_iter=divide_iter, random_seed=self.seed)
        elif self.dataset == 'btad':
    ...
    def visualization(self, image_path_list, gt_list, pr_px, category):
        ...
        if self.vis_type == 'single_norm':
            ...
            for i, path in enumerate(image_path_list):
                ...
                # linux 类型 路径
                # anomaly_type = path.split('/')[-2] # Anomaly / Normal 
                # img_name = path.split('/')[-1] # 图像名
    ...
    def make_category_data(self, category):
        ...
        for divide_iter in range(divide_num):
            ...
            # extract features
            patch_tokens_list = []
            subset_num = len(test_dataset) # 子数据集图像数
            dataset_num += subset_num # 图像总数
            start_time = time.time()
            for image_info in tqdm(test_dataloader):
            # for image_info in test_dataloader: 对数据集的每个图像进行处理
                ...
                with torch.no_grad(), torch.cuda.amp.autocast(): # 自动执行 fp32 -> fp16
                    # 提取图像特征和patch_tokens
                ...
                class_tokens.extend(image_features) # CLS
            ...
            anomaly_maps_r = torch.tensor([]).double() # 不同聚合度下的异常图
            for r in self.r_list:
                ...
                # MSM
                anomaly_maps_l = torch.tensor([]).double() # MSM之后的异常图
                start_time = time.time()
