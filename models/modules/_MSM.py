...
def compute_scores_fast(Z, i, device, topmin_min=0, topmin_max=0.3):
    ...
    Z_ref = torch.cat((Z[:i], Z[i+1:]), dim=0) # 除了当前图像 i 之外的所有图像的特征拼接
    # 计算当前图像的补丁与 Z_ref 中所有补丁之间的距离
    patch2image = torch.cdist(Z[i:i+1], Z_ref.reshape(-1, c)).reshape(patch_num, image_num-1, patch_num)
    patch2image = torch.min(patch2image, -1)[0] # 每个补丁与最近邻图像的最小距离。
    # interval average 
    k_max = topmin_max
    k_min = topmin_min
    if k_max < 1:
...
def MSM(Z, device, topmin_min=0, topmin_max=0.3):
    """
    Args:
        Z (_type_): [N, L, C] [图像数量, 补丁数量, 特征维度]
        device (_type_): CPU or GPU 
        topmin_min (int, optional): 选择最小距离时考虑的最小百分比。. Defaults to 0.
        topmin_max (float, optional): 选择最小距离时考虑的最大百分比。. Defaults to 0.3.
    """
    anomaly_scores_matrix = torch.tensor([]).double().to(device)
    for i in tqdm(range(Z.shape[0])):
    # for i in range(Z.shape[0]):
        anomaly_scores_i = compute_scores_slow(Z, i, device, topmin_min, topmin_max).unsqueeze(0)
